...
function parseQueryAndReturn(operation, tableName, paramsObj) {
  insertQuery = () => {
    switch (tableName) {
      ...
      case 'tracks':
        const { username, name, album, artist, genre, rating, review } =
          paramsObj;
        ...
        album_id
          ? (trackInputObj['album_id'] = album_id)
          : (trackInputObj['album_id'] = query.insert('albums', {
              name: album,
            }));

        const artist_id = query.select('artists', { name: artist });
        artist_id
          ? (trackInputObj['artist_id'] = artist_id)
          : (trackInputObj['artist_id'] = query.insert('artists', {
              name: artist,
            }));

        const genre_id = query.select('genres', { name: genre });
        genre_id
          ? (trackInputObj['genre_id'] = genre_id)
          : (trackInputObj['genre_id'] = query.insert('genres', {
              name: genre,
            }));
  ...
  selectQuery = () => {
    if (tableName === 'userTracks') {
      ...
      const userReviews = query.select('userTracks', { _id: user._id });
